...
def evaluate(dataset: List[str], predictions: List[str], evaluation_types: List[str], dataset_name) -> Dict:
    ...
    for key in metrics.keys():
        metrics[key] /= len(predictions) #min(len(predictions), max_eval)
...
if __name__ == '__main__':
    ...
    data_df= pd.read_csv(args.dataset_file,dtype=object)
    ...
    # print(json.dumps(evaluate(data_df.Output.tolist(), predictions_list, evaluation_types, args.dataset_file)))
    res=evaluate(data_df.output.tolist(), predictions_list, evaluation_types, args.dataset_file)
    print(res)
